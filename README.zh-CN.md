...
记忆快照和检查点现在固定不可变的记忆版本，包括状态、来源类型和证据。严格恢复后的普通召回与上下文会排除后续记忆及冲突；`includeHistory: true` 和按 ID 读取仍可用于历史检查。记忆写入必须使用当前执行世代，传入修订号时也必须与当前值一致。旧数据库会在打开时增量迁移。

记忆快照与合并会保留历史。选定文件的快照提供可移植的字节存储，不负责恢复整个工作目录。可选的系统快照支持取决于文件系统与进程权限。

## 验证情况与限制

**0.1.2 版本的 102 项离线测试全部通过**，新增覆盖严格记忆恢复、过期写入拒绝、超长观察输入断点续读和压缩性能，并保留使用模拟提供方的真实 Pi SDK 接入测试。另已通过全新项目中的独立安装验证。

在同一台机器、256 条诊断事件的合成样例中，首次压缩从 **2,466 毫秒降至 96 毫秒**，完整 token 计量次数从 256 次减为 13 次。这是单次测量，不是延迟保证；详见[可复现性能记录](docs/continuity/benchmarks/README.md)（英文）。

- 已配置的 DeepSeek 模型在提示词调整后通过全部 3 组[真实模型开发用例](docs/continuity/evaluations/README.md)。此前的失败和超时记录均保留。这些用例参与过开发调整，不是独立质量基准；证据链接证明来源，不证明模型理解一定正确。
